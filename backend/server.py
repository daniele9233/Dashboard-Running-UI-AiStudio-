...
async def strava_exchange_code(request: Request):
    ...
    return {"ok": True, "athlete_id": tokens["athlete_id"]}
...
async def update_profile(request: Request):
    ...
    body.pop("id", None)
    body.pop("_id", None)
    await db.profile.update_one({}, {"$set": body}, upsert=True)
    doc = await db.profile.find_one()
    return oid(doc)
...
@app.get("/api/runs")
async def get_runs():
    cursor = db.runs.find().sort("date", -1)
...
async def get_dashboard():
    ...
    # Last run
    last_run = await db.runs.find_one(sort=[("date", -1)])
...
async def get_analytics():
    runs = await db.runs.find().sort("date", -1).to_list(500)
...
async def get_injury_risk():
    ...
    runs = await db.runs.find().sort("date", -1).to_list(60)
    today = dt.date.today()
...
async def get_supercompensation():
    runs = await db.runs.find().sort("date", -1).to_list(30)
...
async def get_badges():
    docs = await db.badges.find().to_list(100)
    if docs:
        return {"badges": oids(docs)}

    # Compute badges from run data
    runs = await db.runs.find().to_list(500)
...
async def get_best_efforts():
    ...
    for r in runs:
        km = r.get("distance_km", 0)
        pace_str = r.get("avg_pace", "99:99")
        try:
            pace_s = int(pace_str.split(":")[0]) * 60 + int(pace_str.split(":")[1])
        except (ValueError, IndexError):
            continue

        # Bucket into standard distances
        for dist_label, dist_min, dist_max in [
            ("1K", 0.9, 1.2), ("5K", 4.8, 5.3), ("10K", 9.7, 10.5),
            ("Half Marathon", 20.5, 22), ("Marathon", 41, 43),
        ]:
            if dist_min <= km <= dist_max:
                if dist_label not in bests or pace_s < bests[dist_label]["pace_s"]:
                    bests[dist_label] = {
                        "distance": dist_label,
                        "pace": pace_str,
                        "pace_s": pace_s,
                        "time": f"{int(r.get('duration_minutes', 0))}:{int((r.get('duration_minutes', 0) % 1) * 60):02d}",
                        "date": r.get("date"),
                    }

    return {"efforts": [{"distance": v["distance"], "pace": v["pace"], "time": v["time"], "date": v["date"]} for v in bests.values()]}


@app.get("/api/heatmap")
async def get_heatmap():
    runs = await db.runs.find({}, {"date": 1, "distance_km": 1}).to_list(500)
    heatmap = {}
    for r in runs:
        d = r.get("date", "")
        if d:
